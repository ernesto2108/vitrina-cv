...
- **Extend-to-intersection (ADR-013 / 08-cv-xx):** `_extend_to_intersection` extiende extremos H y V a su intersección geométrica si el gap ≤ `CV_JUNCTION_EXTEND_PX` (default 40, gt=0, calibrado a ~2000px) y la intersección cae en la prolongación del segmento. Invariante duro: no extender el interior de un segmento ya cubierto; no crear ni borrar muros (misma cardinalidad). Sobre plan-004 (gaps ≈ 0) es no-op verificable. El umbral `0` está prohibido (gt=0).
